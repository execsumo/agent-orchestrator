...
- **ADR-0003 extends what that same listener may serve — it adds no new bind.** The LAN listener may also serve the **browser web UI**: an embedded SPA plus exactly one unauthenticated self-contained `/login` page, with everything else still behind auth. It accepts two additional credentials alongside Bearer — a **web session cookie** and, when explicitly enabled, a **trusted tailnet identity** header injected by `tailscale serve`. Its bind host is configurable (`AO_CONNECT_BIND_HOST`, default `0.0.0.0` so upstream behavior is unchanged). Three rules are load-bearing, each with tests: cookie/identity requests are **CSRF-checked**; cross-origin `/mux` upgrades are **rejected** under cookie or identity auth while Bearer keeps skip-verify for native clients; and **identity trust may only be enabled on a loopback bind** — the daemon refuses to start otherwise, because a header is spoofable by anything that can reach the socket. See `docs/adr/0003-web-ui-on-the-lan-listener.md`.
